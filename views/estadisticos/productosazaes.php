...
if($r){
    $tab1 = mysqli_query($db, "SELECT p.id, p.codigo ,p.nombre, p.medida, m.nombre as 'marca', COALESCE(SUM(pc.cantidad), 0) AS 'total_ventas' "
                            ...
                            . "WHERE cu.fecha BETWEEN '$f1' AND '$f2' AND cu.estado = 'ENTREGADO AZAPAMPA' AND p.est = 'H' AND pc.est = 'H' GROUP BY p.id, p.nombre ORDER BY total_ventas DESC LIMIT $r");
}else{
    $tab1 = mysqli_query($db, "SELECT p.id, p.codigo ,p.nombre, p.medida, m.nombre as 'marca', COALESCE(SUM(pc.cantidad), 0) AS 'total_ventas' "
                            . "FROM producto p LEFT JOIN producto_cuaderno pc ON pc.id_producto = p.id INNER JOIN marca m ON p.id_marca = m.id INNER JOIN cuaderno cu ON cu.id = pc.id_cuaderno "
                            . "WHERE cu.fecha BETWEEN '$f1' AND '$f2' AND cu.estado = 'ENTREGADO AZAPAMPA' AND p.est = 'H' AND pc.est = 'H' GROUP BY p.id, p.nombre ORDER BY total_ventas DESC;");
}
...
<h1>TOP <?=$r?> DE PRODUCTOS ENTREGADOS POR AZAPAMPA</h1>

<table  style="margin-left: 20%;">
    <tr>
        <th>
            <form action="<?=base_url?>views/ereportproductoaza.php" method="GET" target="_blank">
...
<h2>Reporte de Productos Más Vendidos y Entregados en Azapampa del <?=$f1?> al <?=$f2?> </h2>
...
<h2>Gráfico Estadístico de Productos Entregados en Azapampa </h2>
